...
const archive = 'http://127.0.0.1:15213/';
const screenRoot = '../../archives/2026-09-22-v1-185446/screenshots/';
const steps = [
...
function renderBaseline(index, focus=false){
  currentPage=index;const p=pages[index];
  ...
  $('#baseline-panel').innerHTML=`<figure class="baseline-image"><a href="${screenRoot+p.image}" target="_blank" rel="noopener" aria-label="查看${p.name}完整归档截图"><img src="${screenRoot+p.image}" alt="${p.caption}" loading="lazy"></a><figcaption>${p.caption} · 点击查看原尺寸</figcaption></figure><div class="baseline-notes"><h3>${p.title}</h3><h4><span class="status partial">归档已实现</span></h4><p>${p.existing}</p><h4><span class="status research">本轮设计要推进</span></h4><p>${p.next}</p><div class="actions"><a class="primary" href="${archive}#${p.id}" target="_blank" rel="noopener">打开归档页面 ↗</a><button class="secondary" type="button" data-embed>本页展开归档</button></div></div>`;
